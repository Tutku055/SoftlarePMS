...
import { useProfessionsList } from '../hooks/useProfessionsList';
...
export const ProfessionList = () => {
  ...
  const [selectedType, setSelectedType] = useState<Profession | null>(null);
  ...
  const { data, isLoading, isFetching } = useProfessionsList({
  ...
  const handleExport = async () => {
    ...
    const worksheet = workbook.addWorksheet('Professions');

    const allColumns = [
      { id: 'name', label: 'Profession Name', getValue: (t: any) => t.name },
      { id: 'description', label: 'Description', getValue: (t: any) => t.description || '-' },
    ...
    data.items.forEach((t: Profession, index: number) => {
    ...
    a.href = url;
    a.download = `SoftlarePMS_Professions_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const handleRowClick = (id: string | number) => {
    const item = data?.items.find((x: Profession) => x.id === id);
    if (item) {
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3.5 }, maxWidth: 1400, mx: 'auto' }}>
      <PageHeader
        title="Professions"
        subtitle="Manage employee professions and job titles."
        icon={<WorkRounded />}
        actions={
          <Stack direction="row" spacing={1.5}>
            <HasPermission requiredPermission="Professions.Create">
              <Button
                variant="contained"
