...
public sealed class GetYearEndEmployeeStatsQueryHandler(
    IApplicationDbContext context) : IRequestHandler<GetYearEndEmployeeStatsQuery, YearEndStatsDto>
{
    public async Task<YearEndStatsDto> Handle(GetYearEndEmployeeStatsQuery request, CancellationToken cancellationToken)
    {
        ...
        var rolloverLog = await context.YearlyRolloverLogs
        ...
        var timesheetsCounts = await context.MonthlyTimesheets
